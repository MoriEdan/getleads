...
class QrController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow',
                'actions' => array('customize', 'transparent', 'choose', 'generateTransQr', 'generate', 'tag', 'gallery', 'downloadVcard'),
                'users' => array('*'),
            ),
            array('allow',
                'actions' => array('list'),
    ...
    private function addEdit($id = 0) {
        ...
        if ($id) {
            ...
            $model->vcard_data_fname = (isset($attributes['vcard_data_fname']) && $attributes['vcard_data_fname'] != '') ? $attributes['vcard_data_fname'] : '';
            $model->vcard_data_lname = (isset($attributes['vcard_data_lname']) && $attributes['vcard_data_lname'] != '') ? $attributes['vcard_data_lname'] : '';
            $model->vcard_data_telephone = (isset($attributes['vcard_data_telephone']) && $attributes['vcard_data_telephone'] != '') ? $attributes['vcard_data_telephone'] : '';
            $model->vcard_data_cell = (isset($attributes['vcard_data_cell']) && $attributes['vcard_data_cell'] != '') ? $attributes['vcard_data_cell'] : '';
            $model->vcard_data_fax = (isset($attributes['vcard_data_fax']) && $attributes['vcard_data_fax'] != '') ? $attributes['vcard_data_fax'] : '';
        } else {
            $qr = new Qr();
        ...
        if (isset($_POST['QrGenerateForm'])) {
            ...
            $model->company = $_POST['QrGenerateForm']['company'];
            if ($model->validate()) {
                $qr->title = $model->title;
                $qr->type = $model->qr_type;
                $qr->user_id = Yii::app()->user->getId();
                $qr->data = $_POST['QrGenerateForm'];
                $qr->company = $model->company;
//                $qr->vcard_data_fname = $model->vcard_data_fname;
//                $qr->vcard_data_lname = $model->vcard_data_lname;
//                $qr->vcard_data_telephone = $model->vcard_data_telephone;
//                $qr->vcard_data_cell = $model->vcard_data_cell;
                $qr->save();
        ...
        $model_email = new Email('search');
        if ($this->isExportRequest()) { //<==== [[ADD THIS BLOCK BEFORE RENDER]]
            $this->exportCSV($model_email->search(), array('id', 'email_id', 'timestamp', 'ip', 'browser', 'created_at'));
        }
...
//            'params' => array(':user_id' => Yii::app()->user->getId()),
            ...
            $q = new CDbCriteria(array(
                'condition' => 't.company = :company',
                'params' => array(':company' => Yii::app()->user->getState('company')),
                'order' => 't.id DESC',
                'limit' => $limit,
                'offset' => $offset,
            ));
